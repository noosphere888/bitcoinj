...
public class Transaction extends ChildMessage {
    ...
    public synchronized Sha256Hash hashForTaprootWitnessSignature(
            int inputIndex,
            SigHash type,
            byte[][] inputScriptPubKeys,
            Coin[] inputValues,
            boolean anyoneCanPay) {
        int sigHash = TransactionSignature.calcSigHashValue(type, anyoneCanPay);
        return hashForTaprootWitnessSignature(inputIndex, (byte) sigHash, inputScriptPubKeys, inputValues);
    }

    /*
        So far this only has support for basic single-sig P2TR transactions.
     */
    public synchronized Sha256Hash hashForTaprootWitnessSignature(
            int inputIndex,
            byte sigHashType,
        ...
        try {
            ...
            if (!anyoneCanPay) {
                ...
                for(byte[] scriptPubKey : inputScriptPubKeys) {
                    bosHashScriptPubKeys.write(new VarInt(scriptPubKey.length).encode());
                    bosHashScriptPubKeys.write(scriptPubKey);
                }
                ...
                for(Coin value : inputValues) {
                    uint64ToByteStreamLE(BigInteger.valueOf(value.getValue()), bosAmounts);
                }
            ...
            if(basicSigHashType == SigHash.SINGLE.value) {
                bos.write(hashOutputs);
            }
        } catch (IOException e) {
            throw new RuntimeException(e);  // Cannot happen.
